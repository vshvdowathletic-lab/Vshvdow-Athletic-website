...
styles.css      all the visual design (you shouldn't need to touch this)
content.js      ALL the text, prices, spots left, and links — edit this
script.js       the interactive behaviour (you shouldn't need to touch this)
favicon.svg     the little icon in the browser tab
images/         the five photos used on the site
```
...
3. Drag in every file and the whole `images` folder from this package.
...
**Swapping a photo**
Replace the file in `images/` with a new one **using the exact same
file name** (e.g. `hero.jpg`), and it'll update automatically. Keep
new photos under ~500KB so the site stays fast — any free image
compressor (like squoosh.app) will do that in one click.
...
- **Fonts**: the site loads Anton (headlines) and IBM Plex Sans /
  IBM Plex Sans Arabic (body text and Arabic headlines) from Google
  Fonts automatically — no setup needed, this just works once the
